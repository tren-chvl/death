...
int patch_init_array32(int fd, unsigned char *g_stub_code, size_t g_stub_size)
{
	Elf32_Ehdr eh;
	Elf32_Phdr ph;
	...
	lseek(fd, 0, SEEK_SET);
	read(fd, &eh, sizeof(eh));
	Elf32_Off  phoff = eh.e_phoff;
	Elf32_Half phnum = eh.e_phnum;
	Elf32_Phdr dyn_ph;
	int dyn_found = 0;
	for (int i = 0; i < phnum; i++)
	{
	...
	for (off_t off = dyn_ph.p_offset;
		 off < dyn_ph.p_offset + dyn_ph.p_filesz;
		 off += sizeof(dyn))
	{
		lseek(fd, off, SEEK_SET);
		read(fd, &dyn, sizeof(dyn));
	...
	if (!init_array_vaddr || init_array_sz < sizeof(uint32_t))
		return 0;
	Elf32_Phdr code_ph;
	int code_index = -1;
	for (int i = 0; i < phnum; i++)
	...
	lseek(fd, phoff + code_index * sizeof(current), SEEK_SET);
	write(fd, &current, sizeof(current));
	uint32_t stub_vaddr = current.p_vaddr + (stub_off - current.p_offset);
	off_t init_array_off = vaddr_to_file_offset(fd, init_array_vaddr);
	if (init_array_off < 0)
		return 0;
	lseek(fd, init_array_off, SEEK_SET);
...
int patch_init_array(int fd, unsigned char *g_stub_code, size_t g_stub_size)
{
	...
	if (elf.is32)
		return patch_init_array32(fd, g_stub_code, g_stub_size);
	if (elf.is64)
